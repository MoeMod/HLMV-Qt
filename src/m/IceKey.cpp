...
static uint32_t g_pdwIceSBox[4][1024];
...
static const uint32_t g_pdwIcePBox[32] =
{
...
static uint32_t GFMultiply(uint32_t a, uint32_t b, uint32_t m)
{
	uint32_t res = 0;
...
static uint32_t GFExp7(uint32_t b, uint32_t m)
{
	uint32_t x;
...
static uint32_t IcePerm32(uint32_t x)
{
	uint32_t res = 0;
	const uint32_t *pbox = g_pdwIcePBox;
...
static uint32_t IceFunction(uint32_t dwValue, const TIceSubKey *sk)
{
	uint32_t tl, tr;
	uint32_t al, ar;
...
void CIceKey::BuildSchedule(unsigned short *usKeyBuilder, int n, const int *cpiKeyRotation)
{
	...
	for (i = 0; i < 8; ++i)
	{
		...
		for (j = 0; j < 15; ++j)
		{
			uint32_t *pdwCurrentSubKey = &pSubKey->dwValue[j % 3];
...
void CIceKey::SetKey(const uint8_t *pKey)
{
	int i, j;
...
void CIceKey::Encrypt(const uint8_t pPlainText[8], uint8_t pCipherText[8]) const
{
	int i;
	uint32_t l, r;

	l = (((uint32_t)pPlainText[0]) << 24) | (((uint32_t)pPlainText[1]) << 16) | (((uint32_t)pPlainText[2]) << 8) | pPlainText[3];
	r = (((uint32_t)pPlainText[4]) << 24) | (((uint32_t)pPlainText[5]) << 16) | (((uint32_t)pPlainText[6]) << 8) | pPlainText[7];
	...
	for (i = 0; i < 4; ++i)
	{
		pCipherText[3 - i] = (uint8_t)(r & 0xff);
		pCipherText[7 - i] = (uint8_t)(l & 0xff);
...
void CIceKey::Decrypt(const uint8_t pCipherText[8], uint8_t pPlainText[8]) const
{
	int i;
	uint32_t l, r;

	l = (((uint32_t)pCipherText[0]) << 24) | (((uint32_t)pCipherText[1]) << 16) | (((uint32_t)pCipherText[2]) << 8) | pCipherText[3];
	r = (((uint32_t)pCipherText[4]) << 24) | (((uint32_t)pCipherText[5]) << 16) | (((uint32_t)pCipherText[6]) << 8) | pCipherText[7];
	...
	for (i = 0; i < 4; ++i)
	{
		pPlainText[3 - i] = (uint8_t)(r & 0xff);
		pPlainText[7 - i] = (uint8_t)(l & 0xff);
